...
export default function LawyerComplaints() {
  ...
  useEffect(() => {
    caseService.getAll({ limit: 50 }).then((d) => setCases(d.cases)).catch(() => {});
  }, []);

  async function handleSubmit() {
    if (!form.category || !form.subject || !form.description) {
      toast.show('Category, subject, and description are required', 'error');
      return;
    }
    setSaving(true);
    try {
      await complaintService.create(form);
      toast.show('Service request submitted', 'success');
    ...
    <div>
        ...
        <div className="card-body">
          {loading ? (
            ...
            <div className="empty-state">
              ...
              <p>Submit a service request using the button above.</p>
            </div>
          ) : (
            <>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                {complaints.map((c) => (
                  <div
                    key={c.id}
                    style={{ padding: '16px 20px', border: '1px solid var(--gray-200)', borderRadius: 8, cursor: 'pointer' }}
                    onClick={() => { setSelected(c); setViewModal(true); }}
                  >
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 8 }}>
                      <div>
                        <span className={`badge badge-cat-${c.category}`} style={{ marginRight: 8 }}>
                          {c.categoryLabel || categoryLabel(c.category)}
                        </span>
      ...
      >
        <div className="form-group">
          ...
          {selectedCat && <p className="form-hint">{selectedCat.hint} — Handled by: {selectedCat.handledBy === 'clerk' ? 'Registry (clerk)' : 'Court administration'}.</p>}
        </div>
        <div className="form-group">
          ...
          <select className="form-control" value={form.caseId} onChange={(e) => setForm({ ...form, caseId: e.target.value })}>
            <option value="">Not case-specific</option>
            {cases.map((c) => (
